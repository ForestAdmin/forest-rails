      ...
      if Rails.env.development?
        ...
        @collections_sent.each do |collection|
          collection['actions'].each do |action|
            unless action['hooks'].nil?
              collection = get_collection(collection['name'])
              a = get_action(collection, action['name'])
              load = a.hooks.key?(:load) && a.hooks[:load].is_a?(Proc)
              change = a.hooks.key?(:change) && a.hooks[:change].is_a?(Hash) ? a.hooks[:change].keys : []
