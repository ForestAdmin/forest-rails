    ...
    def self.get_client_for_callback_url(callback_url)
      begin
        ...
        if ForestLiana.forest_client_id.nil?
          client_data = Rails.cache.read("#{callback_url}-#{ForestLiana.env_secret}") || nil
          if client_data.nil?
            ...
            Rails.cache.write("#{callback_url}-#{ForestLiana.env_secret}", client_data)
          end
        else
        ...
        Rails.cache.delete("#{callback_url}-#{ForestLiana.env_secret}")
        raise error
      end
    end
  end
