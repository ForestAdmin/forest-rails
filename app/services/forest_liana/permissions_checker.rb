  ...
  class PermissionsChecker
    ...
    def smart_action_allowed?(smart_actions_permissions) 
      if !@smart_action_parameters||
          !@smart_action_parameters['userId'] ||
          !@smart_action_parameters['actionId'] ||
          !smart_actions_permissions ||
          !smart_actions_permissions[@smart_action_parameters['actionId']]
        return false
      end

      @userId = @smart_action_parameters['userId']
      @actionId = @smart_action_parameters['actionId']
      @smart_action_permissions = smart_actions_permissions[@actionId]
      @allowed = @smart_action_permissions['allowed']
      @users = @smart_action_permissions['users']

      return @allowed && (@users.nil?|| @users.include?(@userId));
    end
